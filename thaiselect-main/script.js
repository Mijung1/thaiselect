...
const MIN_ZOOM_FOR_LABELS = 9; 
const customProvinceCenters = {
    'สุราษฎร์ธานี': [8.90, 99.15],
    'พังงา': [8.55, 98.45],
    'กระบี่': [8.10, 98.90],
    'ตราด': [12.40, 102.50],
    'ชลบุรี': [13.30, 101.20],
...
map.getPane('provincePane').style.zIndex = 450; // ตั้งค่าให้สูงกว่าเลเยอร์ปกติ (400)
map.getPane('provincePane').style.pointerEvents = 'none'; // สำคัญมาก: ทำให้เมาส์คลิกทะลุเส้นจังหวัดไปโดนอำเภอได้
...
function updateLabelsVisibility() {
    ...
    // --- 1. จัดการ Label ของอำเภอ (แสดงเฉพาะเมื่อซูมเข้า) ---
        ...
        if (shouldShow) {
            if (!layer.getTooltip()) {
                ...
                layer.bindTooltip(districtName, { 
                    permanent: true, direction: 'center', className: 'district-label', interactive: false 
                });
            }
        } else {
    ...
    // --- 2. จัดการ Label ของจังหวัด (แสดงเฉพาะเมื่อซูมออก) ---
    if (provinceLayer) {
        provinceLayer.eachLayer(function(layer) {
            const props = layer.feature.properties;
            const pName = props[PROVINCE_KEY];
            const layerRegion = provinceToRegionMap.get(pName) || 'Central';

            let shouldShow = !isZoomedIn && enabledRegions.has(layerRegion);
            if (layerRegion === 'Vicinity' && enabledRegions.has('Central') && !isZoomedIn) shouldShow = true;

            // ตรวจสอบว่ามีพิกัดเฉพาะกิจหรือไม่ ถ้ามีให้ใช้พิกัดนั้นแทนพิกัดกึ่งกลางเดิม
            let center = layer.getBounds().getCenter();
            if (customProvinceCenters[pName]) {
                center = L.latLng(customProvinceCenters[pName][0], customProvinceCenters[pName][1]);
            }
            ...
            if (shouldShow) {
                // ใช้การสร้าง Tooltip แบบลอยตัว เพื่อให้เราล็อกตำแหน่งพิกัดได้เอง
                if (!layer._customTooltip) {
                    let labelName = pName === 'กรุงเทพมหานคร' ? 'กรุงเทพฯ' : pName;
                    layer._customTooltip = L.tooltip({
                        permanent: true, direction: 'center', className: 'province-label', interactive: false
                    })
                    .setLatLng(center)
                    .setContent(labelName)
                    .addTo(map);
                }
            } else {
                // ลบ Tooltip ออกเมื่อซูมเข้าหรือไม่ได้อยู่ในมุมมอง
                if (layer._customTooltip) {
...
Promise.all([
    ...
    provinceLayer = L.geoJSON(provinceGeoData, { style: provinceStyle });
...
function updateOutputText() {
    ...
    for (const province of sortedProvinces) {
        ...
        if (selectedDistricts.size === totalCount && totalCount > 0) {
            districtsText = province === 'กรุงเทพมหานคร' ? "ทุกเขต" : "ทุกอำเภอ";
        } else {
